...
class Clase{
    ...
    protected $datos = array(), $ids = array(), $padres = array();
    protected $resultado;
    ...
    protected function obtener_parametros(){
        ...
        for($i = 0; $i < count($this->datos); $i++){
            array_push($this->ids, $this->datos[$i]['id']);
            if(!isset($this->datos[$i]['padre'])){
                array_push($this->padres, 0);
            }else{
                array_push($this->padres, $this->datos[$i]['padre']);
            }
        }
    }
    ...
    protected function procesar(){
        ...
        if($rs_categorias === false){
            ...
            while($row_categoria = $rs_categorias->fetch_assoc()){
                $key = array_search($row_categoria['descripcion'], $this->ids);
                if($key != false){
                    if(!$this->existe_categoria($row_categoria['descripcion'])){
                        $insercion = $this->insertar_categoria($row_categoria, $key);
                        if($insercion <= 0){
                            $num_errores++;
                        }
                    }
    ...
    protected function insertar_categoria($categoria, $key){
        $pad_id = $this->get_id($this->padres[$key]);
        echo $pad_id;
    ...
    protected function get_id($desc){
        $consulta = "SELECT count(*) as cont, categoria_id as id FROM categoria WHERE categoria_descripcion = '$desc'";
