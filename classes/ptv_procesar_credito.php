...
if(isset($_POST["setVentaCredito"])){
    ...
    $credito->procesar_venta_credito($vci,$lci,$vce,$vctb,$vcti,$vcvct,$vct);
    if (isset($_POST["ventaCreditoHabilitado"])){
        $hab = $_POST["ventaCreditoHabilitado"];
        $credito->procesar_venta_habilitado($vci,$hab);
    }
...
if(isset($_POST["setCuotasCredito"])){
    ...
    $credito->procesar_venta_cuotas($p,$vci,$cm,$cfp,$cff,$ce,$cfpa);
}
...
class Credito{
    ...
    public function procesar_venta_habilitado($vci,$hab){
        $insercion_compra = $this->sql_con->prepare("INSERT INTO venta_credito_habilitado(venta_credito_id, venta_credito_habilitado_rut) VALUES                                    (?,?)");
        $insercion_compra->bind_param('is',
          $vci,
          $hab);
        $insercion_compra->execute();
        $insercion_compra->close();
