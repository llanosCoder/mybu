    ...
    $.post("classes/obtener_promociones.php", {
        ...
        function (data) {
            ...
            $.each(data, function (i, datos) {
                ...
                if (datos.f_inicio == '00-00-0000 00:00:00') {
                    promociones += '<td>-</td>';
                } else {
                    var fecha = datos.f_inicio.substr(0, 10);
                    var hora = datos.f_inicio.substr(11, 16);
                    fecha = fecha.split("-").reverse().join("-") + ' ' + hora;
                    promociones += '<td><span style="display:none;">' + datos.f_inicio + '</span>' + fecha + '</td>';
                }
                if (datos.f_termino == '00-00-0000 00:00:00') {
                    promociones += '<td>-</td>';
                } else {
                    var fecha = datos.f_termino.substr(0, 10);
                    var hora = datos.f_termino.substr(11, 16);
                    fecha = fecha.split("-").reverse().join("-") + ' ' + hora;
                    promociones += '<td><span style="display:none;">' + datos.f_termino + '</span>' + fecha + '</td>';
                }
...
$("#btn_aceptar_modal").click(function () {
    ...
    var fecha = $("#f_inicio").val().substr(0, 10);
    var hora = $("#f_inicio").val().substr(11, 16);
    fecha = fecha.split("-").reverse().join("-");
    datos['f_inicio'] = fecha + ' ' + hora;
    var fecha = $("#f_termino").val().substr(0, 10);
    var hora = $("#f_termino").val().substr(11, 16);
    fecha = fecha.split("-").reverse().join("-");
    datos['f_termino'] = fecha + ' ' + hora;
