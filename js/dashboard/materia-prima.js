...
$(document).on('click','.desasignar', function () {
    mostrarModalDesasignar($(this).attr('id'));
});

function cargarMateriaPrima() {
    $(".cl-mcont").html('<div id="loading"><i class="fa fa-spinner fa-2x fa-spin"></i></div>');
    ...
    var tablaMateria = '<div class="table-responsive"><table id="tabla_materia" class="table table-bordered responsive"><thead><tr><th>Asignar</th><th>Código</th><th>Nombre</th><th>Descripción</th><th>Unidad</th><th>Unidad Medida</th><th>Productos Asig</th><th>Stock</th><th>Stock Mínimo</th><th>Edición</th><th>Stock Mín.</th><th>Agregar Stock</th><th>Eliminar</th></tr></thead><tbody>'
    ...
    $.post(url, {
        ...
        function (data) {
            ...
            $.each(data.materia_prima, function (i, datos) {
                ...
                tablaMateria += '<td><a href="#" data-modal="form-lista-asignados" class="info_asignados" onClick="obtenerProductosAsignados(\'' + datos.codigo + '\');"><i class="fa fa-info-circle fa-2x"></i></a><a href="#" data-modal="form-desasignar-materia" class="desasignar" id="' + datos.codigo + '" style="color:red;"><i class="pull-right fa fa-times-circle fa-2x"></i></a></td>';
                ...
                tablaMateria += '<td><a href="javascript:void(0);" onClick="setMarca(\'' + datos.codigo + '\', \'' + datos.nombre + '\', \'' + datos.descripcion + '\', \'' + datos.u_medida + '\');"><i class="fa fa-pencil-square fa-2x editar_marca" data-modal="form-agregar-materia_prima"></i></a></td>';
                tablaMateria += '<td><a href="javascript:void(0);" onClick="setStock(\'' + datos.codigo + '\', \'' + datos.stock_m + '\');"><i class="fa fa-pencil fa-2x editar_stock" data-modal="form-editar-stock_minimo"></i></a></td>';
                tablaMateria += '<td><a href="javascript:void(0);" onClick="setStockAgregar(\'' + datos.codigo + '\');"><i class="fa fa-plus-square fa-2x agregar_stock" data-modal="form-agregar-stock"></i></a></td>';
                tablaMateria += '<td><a href="javascript:void(0);" onClick="eliminarMateriaPrima(\'' + datos.codigo + '\');" style="color:red;"><i class="fa fa-times-circle fa-2x"></i></a></td>';
                tablaMateria += '</tr>';
            });
        }).done(function () {
        tablaMateria += '</tbody></table></div>';
        $('.cl-mcont').html(tablaMateria);
        $('#tabla_materia').dataTable({
            ...
            "oLanguage": {
                ...
                "sInfo": '<div class="col-md-4"><button class="btn btn-default" data-modal="form-asignar-materia" id="btn_asignar_materia"><i class="fa fa-caret-up"></i> Asignar a Productos</button></div></div>',
...
function mostrarModalDesasignar(mId) {
    'use strict';
        var url = 'classes/obtener_productos_asignados_materia_prima.php';
        $('#desasignar_lista_productos').val('0').trigger('change');
        var arrayParametros = ['codigo', 'nombre'];
        selectProductos = '<option value="0">Selecciona una opcion</option>';
        $.post(url, {
            ...
            function (data) {
                data = $.parseJSON(data);
                if(data.resultado == 0){
                    $('#form-desasignar-materia').removeClass('md-show');
                    mostrar_notificacion('Atención', 'No se encontraron productos asignados a esta materia prima', 'warning');
                }else{
                    $.each(data.productos, function (i, datos) {
                        selectProductos += '<option value="' + datos.codigo + '">' + datos.codigo + ' | ' + datos.nombre + '</option>';
                    });
                    $("#desasignar_lista_productos").html(selectProductos);
                }
            }
        ).done(function () {
            $('#btn_aceptar_desasignar').off('click');
            $('#btn_aceptar_desasignar').on('click', function (){
                desasignar(mId);
            });
        });
};

function procesarEliminacion(codigo) {
    'use strict';
    var url = 'classes/eliminar_materia_prima.php';
    $.post(url,
        {
            codigo: codigo
        },
        function (data) {
            ...
            case 1:
                mostrar_notificacion('Éxito', 'Materia prima eliminada exitosamente', 'success');
                //cargarMateriaPrima();
                break;
            case 3:
                mostrar_notificacion('Error', 'No tiene los permisos para realizar esta acción', 'danger');
                break;
            default:
...
function eliminarMateriaPrima(codigo) {
    'use strict';
    for(var i = 0; i < listaProductosAsignados.materia.length; i++){
        
        if(listaProductosAsignados.materia[i].codigo == codigo){
            if(listaProductosAsignados.materia[i].productos.length > 0){
                alertify.confirm('Atención', "Esta materia prima tiene productos asignados, ¿Desea eliminarla de todas maneras?",
                    function () {
                        procesarEliminacion(codigo);
                    },
                    function () {}
                ).set('labels', {
                    ok: 'Sí',
...
function desasignar(mId) {
    'use strict';
    var url = 'classes/desasignar_materia_prima.php',
        pId;
    pId = $('#desasignar_lista_productos').val();
    if(pId == 0){
        mostrar_notificacion('Error', 'Debe seleccionar un producto a eliminar', 'danger');
    }else{
        $.post(url,
            {
                materia: mId,
            ...
            function (data) {
                ...
                case 0:
                    ...
                    break;
                case 1:
                    mostrar_notificacion('Éxito', 'Producto desasignado correctamente', 'success');
                    $('#form-desasignar-materia').removeClass('md-show');
                    cargarMateriaPrima();
                    break;
                default:
