...
$(document).ready(function () {
    ...
    $.post('classes/ptv_getbase_categorias.php', function (data) {
        ...
        console.log('total categorias: '+CATEGORIAS_JSON.length);
        if (CATEGORIAS_JSON.length == 0){
                    ...
                    $.each(PRODUCTOS_JSON, function (i, v) {
                        if (v.nombre.search(new RegExp(buscado, 'gi')) != -1 && $('input[name="b-nombre"]').is(':checked')) {
                            ...
                            if (v.stock_r <= -100 || v.precio_u <= 0) {
                                ...
                                $('#productos-content').append('<button type="button" class="btn btn-default button-square nfn-text item-producto" tittle="description" data-modal="modal-agregar" value="' + codigo + '" style="color:red;" ><p style=" font-size: 9px !important;line-height:0;margin:2px;color:#00a3e4;">SKU.' + codigo + '</p>' + nombre.toLowerCase().substring(0, 19) + '</button>');
                            } else {
                                $('#productos-content').append('<button type="button" class="btn btn-default button-square nfn-text item-producto" tittle="description" data-modal="modal-agregar" value="' + codigo + '"><p style="font-size: 9px !important;line-height:0;margin:2px;color:#00a3e4;">SKU.' + codigo + '</p>' + nombre.toLowerCase().substring(0, 19) + '</button>');
                            }
        ...
        if (e.keyCode == 13) {
            ...
            if (document.getElementById("cantidadItems" + codigo)) {
                ...
                if (verificarStock(codigo, cantidad)) {
                    ...
                    $('#buscador').val('');
                    //mostrar_notificacion('Atención', 'Producto sin Stock', 'danger');
                }
    ...
    function cargarCategorias(padre) {
        ...
        $.each(CATEGORIAS_JSON, function (index, categoria) {
            ...
            if (parseInt(categoria.cat_padre) == parseInt(padre)) {
                ...
                $("#categorías-content").append('<button type="button" class="btn btn-primary button-square-2 nfn-text item-categoria"  data-toggle="tooltip" data-placement="right" title="' + mensaje + '" value="' + categoria.cat_id + '">' + icono + categoria_nombre.toLowerCase() + '</button><br>');
            }
            if (index == lastID) {
        ...
        $("#categorías-content").append('<button class="btn btn-default  button-square-2 nfn-text"  id="btn-volver"><i class="fa fa-reply"></i></button>');
        activar_volver();
        if (padre == 0) {
            $("#opacity-product-mask").fadeTo("fast", 1);
            $("#productos-content").html("");
            ...
            if (parseInt(categoria_producto.cat_id) == parseInt(categoria)) {
                ...
                $.each(PRODUCTOS_JSON, function (index2, producto) {
                    ...
                    if (parseInt(producto.id) == parseInt(id)) {
                        ...
                        nombre = nombre.replace(new RegExp(" ", "g"), "<br>");
                        if (producto.stock_r <= -100 || producto.precio_u <= 0) {
    ...
    function habilitarProducto() {
        ...
        $(".item-producto").click(function () {
            ...
            $.each(resultado, function (index, pro) {
                ...
                $("#product-body").append('<b class="modificable" hidden><h3><div class="input-group input-group-lg"><span class="input-group-addon">$</span><input type="text" placeholder="Ingrese nuevo precio"  class="form-control"></div>');
    ...
    $("#product-add").click(function () {
        ...
        if (document.getElementById("cantidadItems" + codigo)) {
            ...
            if (verificarStock(codigo, cantidad)) {
                $('#cantidadItems' + codigo).val(cantidad + 1);
                $('#cantidadItems' + codigo).change();
                console.log('tengo stock ' + cantidad);
                calcularTotal();
            }
        } else {
            if (verificarStock(codigo, 1)) {
                console.log('tengo stock 1');
                carrito.push(codigo);
                carrito = jQuery.unique(carrito);
                    ...
                    $.each(PRODUCTOS_JSON, function (i, v) {
                        ...
                        if (v.nombre.search(new RegExp(buscado, "gi")) != -1 && $('input[name="b-nombre"]').is(':checked')) {
                            ...
                            if (v.stock_r <= -100 || v.precio_u <= 0) {
                                ...
                                $("#productos-content").append('<button type="button" class="btn btn-default button-square nfn-text item-producto" tittle="description" data-modal="modal-agregar" value="' + codigo + '" style="color:red;" ><p style=" font-size: 9px !important;line-height:0;margin:2px;color:#00a3e4;">SKU.' + codigo + '</p>' + nombre.toLowerCase() + '</button>');
                            } else {
                                $("#productos-content").append('<button type="button" class="btn btn-default button-square nfn-text item-producto" tittle="description" data-modal="modal-agregar" value="' + codigo + '"><p style="font-size: 9px !important;line-height:0;margin:2px;color:#00a3e4;">SKU.' + codigo + '</p>' + nombre.toLowerCase() + '</button>');
                            }
    ...
    $(document).keydown(function (event) {
        ...
        if (!$('input:focus').length > 0 && event.which == 73) {
            ...
            $("#buscador").focus();
        } else if (search_state == "" && event.which == 13 && $("#modal-pagar").css('visibility') == 'hidden' && $("#modal-agregar").css('visibility') == 'visible') {
            event.preventDefault();
            $("#product-add").click();
    ...
    function cargarNotificaciones() {
        $('#notification-fa').show();

        var total_ss = 0;
        var total_bm = 0;
        var total_sp = 0;
        $.each(PRODUCTOS_JSON, function (i, producto) {
            if (parseInt(producto.stock_r) <= 0) {
                total_ss++;

            }
            if (parseInt(producto.stock_m) > parseInt(producto.stock_r) && parseInt(producto.stock_r) > 0) {
                total_bm++;
            }
            
            if (parseInt(producto.precio_u) <= 0) {
                total_sp++;
            }
        });
        ...
        var notifications = 0;
        if (total_ss > 0) {
            $('#detalleNotificacion').append(' <li><a href="stock.html"><i class="fa fa-info danger"></i><b>' + total_ss + '</b> productos sin stock<span class="date">Agregar Stock</span></a></li>');
            notifications++;
        }

        if (total_bm > 0) {
            $('#detalleNotificacion').append(' <li><a href="stock.html"><i class="fa fa-info danger"></i><b>' + total_bm + '</b> productos bajo el stock mínimo<span class="date">Agregar Stock</span></a></li>');
            notifications++;
        }
        if (total_sp > 0) {
            $('#detalleNotificacion').append(' <li><a href="precios.html"><i class="fa fa-info danger"></i><b>' + total_sp + '</b> productos sin un precio asignado<span class="date">Agregar Precio</span></a></li>');
            notifications++;
        }

        if (notifications > 0) {
            $('#hayNotificacion').html('<i class="fa fa-bell"></i><span class="bubble">' + notifications + '</span>');
        }
