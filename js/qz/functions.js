...
function qzNoConnection() {
    mostrar_notificacion('Error', 'Debe iniciar módulo QZ <br><a href="http://www.qz.io/download/"><i class="fa fa-download"></i> Descargar</a>', 'danger');
...
function sanear_string(cadena) {
    ...
    var destino = "AAAAAEEEEIIIIOOOOUUUUaaaaaeeeeiiiioooouuuunncc_____";
    for (var i=0; i<origen.length; i++) {
        cadena = cadena.replace(origen.charAt(i), destino.charAt(i));
    }
    return cadena;
}


function changeSize(text,total,place,corte){
    total = parseInt(total);
    text = text.toString();
    var new_text = text;
    var padding = '';
    if (text.length < total){
        for (i = 0; i<(total-text.length) ; i++){
            padding += ' ';
        }
    }
    else{
        if (corte=='f')
            new_text = text.substring((text.length - total - 1),text.length);
        else
            new_text = text.substring(0,total);
    }
    if (place=='b'){
        new_text = padding + new_text;
    }
    else{
...
function printESCP(objeto,venta) {
    if (notReady()) { return; }
    // Append a png in ESCP format with single pixel density
    qz.appendImage(getPath() + "src/pixel.png", "ESCP", "single");
    // Automatically gets called when "qz.appendImage()" is finished.
    window["qzDoneAppending"] = function() {
        // Append the rest of our commands
        //Reset Printer
        ...
        today = dd+'/'+mm+'/'+yyyy;
        //Center text
        qz.append('\x1B\x61\x31');
        qz.append("VALE DE VENTA\r\n");
        //(Character font B selected)
        qz.append("\x1B\x21\x01");
        qz.append("Vendedor: " + sanear_string($('#nombre_usuario').html()) + "       Fecha:" + today + " \r\n");
        //Like Table Columns
        qz.append('\x1B\x61\x30');
        qz.append("----------------------------------------------------------------\r\n");
        //Like Table Columns
        
        qz.append("Cantidad      Codigo            Nombre            Valor\r\n");
        //LIKE Table Rows
        $.each(objeto, function (i, p) {
            var cantidad = changeSize(p.producto_cantidad,8,'b','f');
            var codigo = changeSize(p.producto_codigo,16,'a','f');
            var nombre = changeSize(p.producto_nombre,16,'a','a');
            var precio =  changeSize((p.producto_precio * p.producto_cantidad),7,'b','f');
           qz.append(cantidad + "      " + codigo + "  " + nombre + precio + "\r\n");
        });
        
        qz.append("----------------------------------------------------------------\r\n");
        qz.append("Sub-total:  " + formatearNum(venta.venta_bruto) + "\r\n");
        qz.append("Descuentos: " + formatearNum(venta.venta_descuentos) + "\r\n");
        qz.append("Total:      " +formatearNum(venta.venta_neto) + "\r\n");
        qz.append(" \r\n");
        //Full cut paper
        qz.append("\x1D\x56\x41");
        //restart printer for next use
        ...
        $.each(objeto, function (i, p) {
            ...
            var cantidad = changeSize(p.producto_cantidad,8,'b');
            var codigo = changeSize(p.producto_codigo,16,'a');
            var nombre = changeSize(p.producto_nombre,16,'a');
            var precio =  changeSize((p.producto_precio * p.producto_cantidad),7,'b');
           qz.append(cantidad + "      " + codigo + "  " + nombre + precio + "\r\n");
        });
