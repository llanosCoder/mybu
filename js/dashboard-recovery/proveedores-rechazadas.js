var productosCodigo = [], productosCantidad = [], productosStock = [], productosMinimo = [], indiceOrden = 5;

$(document).ready(function documentoListo() {
    obtenerOrdenesVencidas(indiceOrden);
});
...
function obtenerOrdenesVencidas(orden) {
    var url = 'classes/obtener_ordenes.php';
    var table = '<table class="table table-bordered" id="tabla_ordenes" class="table table-bordered"><thead><tr><th>Voucher<i></th><th>Fecha creación</th><th>Fecha Vencimiento </th><th>Total Bruto</th><th>Descuentos</th><th>Total Final</th><th>Razón rechazo</th><th>Detalle</th></tr></thead><tbody>';
    $.post(url, {
            orden: orden
        },
        function datosOrdenes(data) {
            ...
            $.each(data, function (i, datos) {
                ...
                if(razon == '')
                    razon = '-';
                table += '<td>' + razon + '</td>';
                table += '<td><a href="#"><i class="fa fa-list-alt fa-2x detalle_orden" data-modal="form-detalle-orden" onClick="obtenerProductosOrden(\'' + datos.voucher + '\');"></i></a></td></tr>';
            });
        }
        ...
        $("#tabla_ordenes").dataTable({
            ...
            "oLanguage": {
                ...
                "sZeroRecords": "No se encontraron ordenes de compra rechazadas",
                "sEmptyTable": "No hay ordenes de compra rechazadas",
...
function obtenerProductosOrden(voucher, total) {
    $("#form-agregar-marca-body").html('<div id="loading"><i class=fa fa-spinner fa-spin fa-2x"></i></div>');
    ...
    $.post(url, {
        ...
        $("#form-agregar-marca-body").html(body);
